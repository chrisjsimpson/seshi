...
//queue the sending of a json responce
...
function connect(info) {
    ...
    connectFirstParty = function() {
        log("Connect first party called");
    ...
    connectSecondParty = function(){
        log("Connect second Party called");
    ...
    // partner of the id in info.postData.i
function sendMessage(info){
    ...
    if(typeof(postData.message) === "undefined"){
        webrtcError("No message receives", res);
        return;
    }
    if(typeof(postData.id) === "undefined"){
        webrtcError("No id recieved with message", res);
        return;
    }
    ...
    webrtcResponse("Savingmessage ***" + + postData.message + "*** for delivery to id " + partner[postData.id], res);
}
...
function getMessages(info){
    ...
    log("Sending messages ***" + 
        JSON.stringify(messagesFor[postData.id])+ "*** to id " + postData.id);
